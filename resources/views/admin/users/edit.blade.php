...
@section('content_header')
    <h1 class="animate-slide-in">Edit Data Admin</h1>
@stop
...
<div class="card shadow-lg animate-fade-in" style="border-radius:10px;overflow:hidden;">
    <div class="card-header bg-blue-eduself text-white" style="border-bottom:3px solid #192334;">
        <h3 class="card-title" style="font-weight:600;">
            <i class="fas fa-user-shield mr-2"></i>Form Edit Admin
        </h3>
        <div class="card-tools">
            <a href="{{ route('users.index') }}" class="btn btn-light btn-sm btn-animated">
                <i class="fas fa-arrow-left mr-1"></i>Kembali
            </a>
        </div>
    </div>
    <div class="card-body">
        <form action="{{ route('users.update', $user->id) }}" method="POST" class="animate-fade-in">
            @csrf
            @method('PUT')
            
            <div class="form-group">
                <label for="name" class="font-weight-500">Nama</label>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    <label for="password" class="font-weight-500">Password Baru (opsional)</label>
                    <input type="password" name="password" id="password"
                           class="form-control input-animated @error('password') is-invalid @enderror"
                           placeholder="Minimal 8 karakter">
                    <small class="form-text text-muted">Kosongkan jika tidak ingin mengubah password.</small>
                    @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="form-group col-md-6">
                    <label for="password_confirmation" class="font-weight-500">Konfirmasi Password Baru</label>
            ...
            <div class="mt-4 d-flex align-items-center gap-2">
                ...
                <a href="{{ route('users.index') }}" class="btn btn-secondary btn-animated px-4">
