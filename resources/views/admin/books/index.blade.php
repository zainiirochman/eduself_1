...
@section('content_header')
    <h1 class="animate-slide-in">Data Buku</h1>
@stop
...
<div class="card shadow-lg animate-fade-in" style="border-radius:10px;overflow:hidden;">
    <div class="card-header bg-blue-eduself text-white" style="border-bottom:3px solid #192334;">
        <h3 class="card-title" style="font-weight:600;"><i class="fas fa-book mr-2"></i>Daftar Buku</h3>
        <div class="card-tools">
            <a href="{{ route('books.create') }}" class="btn btn-light btn-sm btn-animated shadow-sm">
    ...
    <div class="card-body">
        ...
        <form method="GET" action="{{ route('books.index') }}" class="mb-4 search-form">
                ...
                <tbody>
                    ...
                    <tr class="table-row-animated" data-aos="fade-up" data-aos-delay="{{ $key * 40 }}">
                        <td class="text-center align-middle">
                            <span class="badge badge-secondary badge-pill">{{ $books->firstItem() + $key }}</span>
                        </td>
                        <td class="align-middle">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <span class="font-weight-500">{{ $book->title }}</span><br>
                                    <small class="text-muted">ID: {{ $book->id }}</small>
                                </div>
                            </div>
                        </td>
                        <td class="align-middle">
                            <span class="badge badge-info px-3 py-2 badge-soft">{{ $book->category->name }}</span>
                        </td>
                        <td class="align-middle">{{ $book->author }}</td>
                        <td class="align-middle">{{ $book->publisher }}</td>
                        <td class="align-middle">{{ $book->year }}</td>
                        <td class="align-middle">
                            <span class="badge {{ $book->stock > 0 ? 'badge-success' : 'badge-danger' }} px-3 py-2 badge-soft">
                                {{ $book->stock }}
                            </span>
                        </td>
                        <td class="text-center align-middle">
